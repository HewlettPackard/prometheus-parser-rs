...
fn parse_parentheses() -> Result<()> {
    assert_eq!(
        ...
        Operator::new(
            OperatorKind::GreaterThan,
            Group::new(Selector::new().metric("f").span((1, 2)).wrap())
                .span((0, 3))
                .wrap(),
            Selector::new().metric("bar").span((6, 9)).wrap()
        )
    ...
    assert_eq!(
        ...
        Operator::new(
            OperatorKind::Add,
            Group::new(Selector::new().metric("some").span((1, 5)).wrap())
                .span((0, 6))
                .wrap(),
            Group::new(Selector::new().metric("more").span((8, 12)).wrap())
                .span((7, 13))
                .wrap(),
        )
        .span((6, 7))
...
fn parse_weird_floats() -> Result<()> {
    ...
    match parse_expr("Inf + -1")? {
        Expression::Operator(o) => {
            match *o.lhs {
                Expression::Float(f) => {
                    assert!(f.is_infinite() && f.is_sign_positive(), "must be +inf")
                }
                _ => assert!(false, "must be float"),
            }
