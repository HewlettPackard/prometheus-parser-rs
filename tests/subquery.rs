...
fn parse_selector_subquery() -> Result<()> {
    ...
    // invalid whitespace
    assert_eq!(parse_expr(r#"foo[ 5m:]"#).is_err(), true);

    // valid whitespace
    assert_eq!(parse_expr(r#"foo[5m  :  30s   ]"#).is_ok(), true);

    assert_eq!(
        parse_expr(r#"foo[5m:]"#)?,
        Selector::new()
            .metric("foo")
            .subquery(Subquery::new(PromDuration::Minutes(5)).span((3, 8)))
            .span((0, 8))
            .wrap()
    );
    ...
    assert_eq!(
        ...
        Selector::new()
            .metric("foo")
            .subquery(
                Subquery::new(PromDuration::Minutes(5))
...
fn parse_group_subquery() -> Result<()> {
    ...
    assert_eq!(
        parse_expr(r#"(foo)[5m:]"#)?,
        Group::new(Selector::new().metric("foo").span((1, 4)).wrap())
            .subquery(Subquery::new(PromDuration::Minutes(5)).span((5, 10)))
            .span((0, 10))
            .wrap()
    );
...
fn parse_function_subquery() -> Result<()> {
    // not a subquery
    assert_eq!(parse_expr(r#"sum(foo)[5m]"#).is_err(), true);

    // invalid whitespace
    assert_eq!(parse_expr(r#"sum(foo)[ 5m:]"#).is_err(), true);

    // valid whitespace
    assert_eq!(parse_expr(r#"sum(foo)[5m  :  30s   ]"#).is_ok(), true);

    assert_eq!(
        parse_expr(r#"sum(foo)[5m:]"#)?,
        Function::new("sum")
            .arg(Selector::new().metric("foo").span((4, 7)).wrap())
...
fn return_value_subquery() -> Result<()> {
    ...
    assert_eq!(
        ...
        ReturnValue {
            kind: ReturnKind::InstantVector,
            label_ops: vec![LabelSetOp::clear(
                expr.clone(),
